...
export async function fetchLeaderboard(): Promise<{
	...
	try {
		...
		console.log(
			`[GITHUB] Fetch complete. Evaluating ${allNodes.length} total items against REPOS list...`
		);
		...
		for (const node of allNodes) {
			...
			const acceptedLabel = labels.find(
				(l) =>
					l.name &&
					validPrefixes.some((prefix) => l.name.toLowerCase().includes(prefix.toLowerCase()))
			);

			// If not AI, it MUST have an accepted label
			if (!isAI && !acceptedLabel) {
				console.log(
					`[DEBUG] Rejected PR #${node.number} in ${repoNameWithOwner}: No accepted label found and not AI.`
				);
				continue;
			}
			...
			if (!isPR && isIgnoredAuthor) {
				console.log(
					`[DEBUG] Rejected Issue #${node.number} in ${repoNameWithOwner}: Author role '${authorRole}' is ignored for issues.`
				);
				continue;
			}
			...
			const finalRepoName = repoConfig
			...
			if (isSpecialRepo) {
				...
				const specialLabel = labels.find(
					(l) => l.name && safeSpecialPrefix && l.name.toLowerCase().startsWith(safeSpecialPrefix)
				);
				if (specialLabel) {
			...
			console.log(
				`[DEBUG] Successfully assigned ${points} pts to ${login} for PR/Issue #${node.number}`
			);
		}

		console.log(
			`[GITHUB] Processing completed successfully. Total accepted items: ${stats.acceptedCount}`
		);
	} catch (e: unknown) {
